...
def get_land_price_analytics(history_data):
    ...
    if len(df) < 5:
        analytics["forecasted_price"] = (
            int(df["price_inr"].iloc[-1]) if not df.empty else None
        )
        return analytics
    ...
    analytics["regression_line"] = [
        {"date": row["record_date"], "price": int(row["reg_price"])}
    ...
    analytics["moving_average"] = [
        {"date": row["record_date"], "price": int(row["ma_price"])}
        for index, row in df.dropna(subset=["ma_price"]).iterrows()
    ]
...
def calculate_fair_option_premium(predicted_price, strike_price):
    ...
    if intrinsic_value > 0:
        fair_premium = int(intrinsic_value * 0.2)
    else:
        fair_premium = 0
