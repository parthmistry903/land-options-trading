...
def register():
    ...
    if request.method == "POST":
        ...
        sql = (
            ...
            "VALUES (%s, %s, %s, %s, CURDATE(), 0, %s, 'user')"
        )
...
@login_required
def map_page():
    cities_data = execute_query(
        "SELECT DISTINCT city FROM Parcels WHERE city IS NOT NULL",
        fetch_all=True
    )
...
def list_users():
    ...
    sql = (
        f"SELECT user_id, username, full_name, balance_cash FROM Users {where_sql} "
        f"ORDER BY registration_date DESC LIMIT %s OFFSET %s"
    )
...
def view_user(user_id):
    ...
    parcels = execute_query(
        "SELECT parcel_id, address, city, base_price_inr FROM Parcels WHERE owner_user_id = %s",
        (user_id,),
        fetch_all=True
    )
    trades = execute_query(
        ...
        "WHERE T.buyer_user_id = %s OR T.seller_user_id = %s ORDER BY T.trade_date DESC",
        (user_id, user_id),
        fetch_all=True
    )
...
def delete_user(user_id):
    ...
    parcels_count = execute_query(
        "SELECT COUNT(*) as count FROM Parcels WHERE owner_user_id = %s",
        (user_id,),
        fetch_all=False
    )
    options_count = execute_query(
        "SELECT COUNT(*) as count FROM Options WHERE seller_user_id = %s OR buyer_user_id = %s",
        (user_id, user_id),
        fetch_all=False
    )
    trades_count = execute_query(
        "SELECT COUNT(*) as count FROM Trades WHERE seller_user_id = %s OR buyer_user_id = %s",
        (user_id, user_id),
        fetch_all=False
    )

    if (
        (parcels_count and parcels_count.get("count", 0) > 0)
        or (options_count and options_count.get("count", 0) > 0)
        or (trades_count and trades_count.get("count", 0) > 0)
    ):
...
def list_parcels():
    ...
    total_result = execute_query(
        "SELECT COUNT(*) as total FROM Parcels P JOIN Users U ON P.owner_user_id = U.user_id" + where_sql,
        tuple(params),
        fetch_all=False
    )
    ...
    sql = (
        f"SELECT P.*, U.username as owner_name FROM Parcels P JOIN Users U ON P.owner_user_id = U.user_id {where_sql} "
...
def view_parcel(parcel_id):
    parcel = execute_query(
        "SELECT P.*, U.username as owner_name FROM Parcels P JOIN Users U ON P.owner_user_id = U.user_id WHERE P.parcel_id = %s",
        (parcel_id,),
        fetch_all=False
    )
    if not parcel:
    ...
    history = execute_query(
        "SELECT record_date, price_inr FROM Price_History WHERE parcel_id = %s ORDER BY record_date ASC",
        (parcel_id,),
        fetch_all=True
    )
    ...
    active_price = (
        ...
        if parcel.get("listing_price_inr")
        else (history[-1]["price_inr"] if history else parcel["base_price_inr"])
    )
...
def toggle_sale(parcel_id):
    parcel = execute_query(
        "SELECT owner_user_id, is_for_sale FROM Parcels WHERE parcel_id = %s",
        (parcel_id,),
        fetch_all=False
    )
    ...
    if new_status:
        ...
        execute_query(
            "UPDATE Parcels SET is_for_sale = %s, listing_price_inr = NULL WHERE parcel_id = %s",
...
def buy_parcel(parcel_id):
    ...
    parcel = execute_query(
        "SELECT base_price_inr, listing_price_inr, owner_user_id, is_for_sale FROM Parcels WHERE parcel_id = %s",
        (parcel_id,),
        fetch_all=False
    )
    if not parcel:
...
def list_options():
    ...
    count_sql = (
        "SELECT COUNT(*) as total FROM Options O JOIN Parcels P ON O.parcel_id = P.parcel_id "
        "JOIN Users U_Seller ON O.seller_user_id = U_Seller.user_id "
        "LEFT JOIN Users U_Buyer ON O.buyer_user_id = U_Buyer.user_id" + where_sql
    )
    total_records_result = execute_query(count_sql, tuple(params), fetch_all=False)
    total = total_records_result["total"] if total_records_result else 0
    total_pages = max(1, (total + per_page - 1) // per_page)
...
def buy_option(option_id):
    ...
    option = execute_query(
        "SELECT premium_inr, seller_user_id, status FROM Options WHERE option_id = %s AND expiry_date >= CURDATE()",
        (option_id,),
        fetch_all=False
    )
